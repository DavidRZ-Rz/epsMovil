...
const EditarPerfil = () => {
  ...
  const userData = route.params?.usuario?.user || {};
  ...
  useEffect(() => {
    if (route.params?.usuario) {
      setFormData({
        name: route.params.usuario.user.name || '',
        email: route.params.usuario.user.email || '',
        password: '',
        confirmPassword: ''
      });
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        Alert.alert('Error', result.error.message || 'Error al actualizar el perfil');
      }
    } catch (error) {
